...
class AirSimDataCollector:
    ...
    def fly_control(self, duration=600):
        """通过Redis接收位姿命令控制无人机飞行并采集数据"""
        print("开始无人机飞行和数据采集")
        ...
        while time.time() - start_time < duration:
            # 从Redis获取位姿命令
            pose_data = self.redis_client.get(self.pose_channel)
            if pose_data:
                try:
                    pose_dict = json.loads(pose_data)
                    x = pose_dict.get('x', 0)
                    y = pose_dict.get('y', 0)
                    z = pose_dict.get('z', -2)
                    ...
                    print(f"接收的无人机位姿: x={x}, y={y}, z={z}, pitch={pitch}, roll={roll}, yaw={yaw}")
                    
                    # 设置无人机位姿
                    pose = airsim.Pose(
                        airsim.Vector3r(x, y, z),
                        airsim.to_quaternion(pitch=pitch, roll=roll, yaw=yaw)
                    )
                    self.client.simSetVehiclePose(pose, True)

                    time.sleep(1) # 暂停1秒,确保无人机位姿设置到位了
                    ...
                    print("Invalid JSON data received from Redis")
...
if __name__ == "__main__":
    try:
        ...
        collector.fly_control(duration=600)  # 运行60秒
